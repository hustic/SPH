...
void SPH_main::update_particle()
{
	vector<SPH_particle> half_particle_list = particle_list;

	int num = half_particle_list.size();

	for (int i = 0; i < num; i++)
	{
		for (int k = 0; k < 2; k++)
		{
			half_particle_list[i].x[k] = particle_list[i].x[k] + 0.5 * dt * particle_list[i].v[k];
			half_particle_list[i].v[k] = particle_list[i].v[k] + 0.5 * dt * particle_list[i].a[k];
		}
		half_particle_list[i].rho = particle_list[i].rho + 0.5 * dt * particle_list[i].D;
	}

	for (int i = 0; i < num; i++)
	{
		neighbour_iterate(&half_particle_list[i]);
	}
	...
	for (int i = 0; i < num; i++)
	{
		for (int k = 0; k < 2; k++)
		{
			particle_list[i].x[k] = particle_list[i].x[k] + dt * half_particle_list[i].v[k];
			particle_list[i].v[k] = particle_list[i].v[k] + dt * half_particle_list[i].a[k];
		}
		particle_list[i].rho = particle_list[i].rho + dt * half_particle_list[i].D;
		particle_list[i].P = B * (pow((particle_list[i].rho / rho0), gamma) - 1);
	}
}